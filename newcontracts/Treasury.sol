...
contract Treasury is AccessControl, ReentrancyGuard {
    ...
    constructor(address admin, address eudrComplianceAddress) {
    ...
    function deposit() external payable {
        ...
        // Verificar el cumplimiento EUDR del productor antes de depositar fondos
        require(
            eudrCompliance.isProducerCompliant(msg.sender),
            "Treasury: El productor no cumple con la normativa EUDR"
        );
    ...
    function spendFunds(
        address user,
        uint256 amount
    ) external onlyRole(TREASURY_SPENDER_ROLE) {
        require(
    ...
    function addTreasuryAdmin(
        address account
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _grantRole(TREASURY_ADMIN_ROLE, account);
    }

    function removeTreasuryAdmin(
        address account
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function addTreasurySpender(
        address account
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _grantRole(TREASURY_SPENDER_ROLE, account);
    }

    function removeTreasurySpender(
        address account
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
